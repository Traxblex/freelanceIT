...
<div class="max-w-7xl mx-auto px-6 lg:px-8 mt-24 mb-12">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">
            Ravi de vous revoir, <?= ($user['prenom'] ?? 'Utilisateur') ?>,
        </h2>

        <?php if ($role === 'client'): ?>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="p-6 bg-gray-50 rounded-xl border border-gray-100">
                    <h3 class="text-lg font-bold text-gray-800 mt-8 mb-4">Mon entreprise</h3>
                    <p class="text-gray-700 text-sm mt-1"><?= ($entreprise['Nom_entreprise'] ?? 'Aucune entreprise associée') ?></p>
                    <p class="text-gray-700 text-sm mt-1"><?= ($entreprise['adresse'] ?? 'Adresse non disponible') ?></p>
                    <p class="text-gray-700 text-sm mt-1"><?= ($entreprise['description'] ?? 'Description non disponible') ?></p>
                    <p class="text-gray-700 text-sm mt-1">SIRET : <?= ($entreprise['SIRET'] ?? 'Non disponible') ?></p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                <div class="p-6 bg-gray-50 rounded-xl border border-gray-100">
                   <p class="text-sm font-medium text-gray-500 uppercase">nombre de candidatures</p>
